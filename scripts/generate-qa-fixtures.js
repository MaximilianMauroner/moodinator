...
function createQaFixture(count) {
  const latest = Date.UTC(2026, 8, 1, 12);
  return Array.from({ length: count }, (_, index) => ({
    timestamp: latest - index * 6 * 60 * 60 * 1000,
    // An explicit offset makes the same fabricated entry addressable while a
    // QA emulator travels between timezones.
    utcOffsetMinutes: 0,
    mood: index % 11,
...
function writeQaFixture(count, output) {
...
module.exports = { createQaFixture, writeQaFixture };
